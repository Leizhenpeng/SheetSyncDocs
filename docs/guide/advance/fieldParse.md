...
我先一次性给出所有类型的名称与对应声明关键字，后面在依次讲解每一种类型。
|**类型约束** | **显示标识**              | **隐形推断标识**                                                 
|:------------|:-----------------------|:------------------------------------------------------------
**[文本](#文本)**     | text | 默认匹配                                                       
...
> 此约束类型的单元格解析后会用来控制图层的**阴影**。

以下`表头内容`均为有效显示声明，标识此列单元格为阴影类型
...
以下`单元格的内容`均为有效隐式推断，解析器可以推断出此列单元格为阴影类型
...
>此约束类型的单元格解析后会用来控制**图层的描边**。

以下`表头内容`均为有效显示声明，标识此列单元格为描边类型
